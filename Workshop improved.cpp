...
int CalculateMaxWorkshops(Available_Workshops* ptr){
    ...
    for(auto i=ptr->arr.begin();i<ptr->arr.end();i++){
        ...
        for(auto j=i+1;j<ptr->arr.end();j++){     
            ...
            if(((i->end_time < j->end_time) && (i->end_time > j->start_time))||((i->start_time < j->end_time)&&(i->start_time > j->start_time))||(i->start_time == j->start_time)){
                
                ptr->arr.erase(j); 
                j--;               
            }
            // else if((i->start_time < j->end_time)&&(i->start_time > j->start_time)){
            //     ptr->arr.erase(j);                
            // }
